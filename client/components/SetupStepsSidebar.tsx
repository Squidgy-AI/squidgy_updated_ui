        ...
        {updatedSteps.map((step) => (
          ...
          >
            ...
            <div className="flex-1">
              ...
              {/* Retry Facebook Connection Button - only show for step 6 */}
              {step.id === 6 && step.status === "current" && onRetryFacebook && (
                <button
                  onClick={onRetryFacebook}
                  disabled={isRetrying}
                  className="mt-2 w-full bg-white border-2 border-orange-400 text-orange-600 font-medium text-xs py-2 px-3 rounded-md hover:bg-orange-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
                  {isRetrying ? (
                    <>
