...
const SetNewPassword: React.FC = () => {
  ...
  // Check authentication and handle password reset
  useEffect(() => {
    const checkAuthStatus = async () => {
      try {
        ...
        if (!session?.user) {
          ...
          const hasAuthParams = urlParams.get('type') === 'recovery' || 
      ...
      if (!session?.user) {
        ...
        if (accessToken) {
          try {
            ...
            console.error('Error with token session:', tokenError);
          }
        } else if (code) {
          try {
            ...
            console.error('Error with code exchange:', codeError);
          }
        }
      }
      
      // Use Supabase's updateUser directly
        ...
        <div className="flex-1 flex flex-col justify-center px-8 sm:px-12 lg:px-16 xl:px-20 2xl:px-24 max-w-[600px]">
          <div className="w-full max-w-[400px] mx-auto">
            ...
            <div className="text-center">
              ...
              <p className="text-[15px] text-center text-[#4A5565] font-['Open_Sans'] mb-8">
                Your password has been updated. Redirecting you to sign in...
              </p>
              
              <div className="flex justify-center">
                <div className="w-8 h-8 border-4 border-[#5E17EB] border-t-transparent rounded-full animate-spin"></div>
              </div>
            </div>
          </div>
        </div>
